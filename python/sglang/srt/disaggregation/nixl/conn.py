...
class NixlKVManager(CommonKVManager):
    def __init__(
        ...
        if self.disaggregation_mode == DisaggregationMode.PREFILL:
            ...
            for i, queue in enumerate(self.transfer_queues):
                threading.Thread(
                    target=self.transfer_worker, args=(queue, i), daemon=True
                ).start()
    ...
    def set_kv_buffer_tensors(self, k_buffers: list, v_buffers: list, page_size: int):
        ...
        if (
            ...
            # One staging buffer per transfer queue/worker so concurrent
            # workers don't race on the same gather destination.
    ...
    def transfer_worker(self, queue: FastQueue, queue_idx: int = 0):
        # Each worker owns a dedicated staging buffer (lazily resolved on the
        # first chunk because buffers are created in set_kv_buffer_tensors()).
        staging_buffer = None
        while True:
            ...
            try:
                ...
                if (
                    self.enable_staging
                    and staging_buffer is None
                    and self._staging_ctx is not None
                    and self._staging_ctx.buffers
                    and queue_idx < len(self._staging_ctx.buffers)
                ):
                    staging_buffer = self._staging_ctx.buffers[queue_idx]
    ...
    def _get_staging_strategy(self, staging_buffer):
        """Lazily create or return a cached PrefillStagingStrategy per buffer.

        Each transfer worker uses its own staging buffer (see transfer_worker)
        so we key strategies by buffer identity rather than as a singleton.
        """
        if not hasattr(self, "_staging_strategies"):
            self._staging_strategies: Dict[int, object] = {}
        key = id(staging_buffer)
        strategy = self._staging_strategies.get(key)
        if strategy is None:
            from sglang.srt.disaggregation.common.staging_handler import (
                PrefillStagingStrategy,
            )

            strategy = PrefillStagingStrategy(self, staging_buffer)
            self._staging_strategies[key] = strategy
        return strategy
        ...
        for peer_name, messages in notif_map.items():
            for msg in messages:
                ...
                if tag == "kv":
                    ...
                    self._handle_aux_notification(components, room)
                elif tag == "state":
    ...
    def _handle_aux_notification(self, components, room: int):
